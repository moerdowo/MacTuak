            ...
            VStack(spacing: 0) {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 16) {
                        Text("MacTuak runs Windows applications using Wine and related open-source software. The notices below fulfill those components' license requirements.")
                        ...
                        section("winetricks") {
                            Text("Optional components are installed via winetricks (LGPL-2.1), downloaded at runtime. Microsoft redistributables it fetches are under Microsoft's own EULAs and are not bundled by MacTuak.")
                                .font(.system(size: 12)).foregroundStyle(p.text)
                            HStack(spacing: 14) {
                        ...
                        section("Bundled tools") {
                            Text("MacTuak ships cabextract (GPLv3) and 7-Zip / p7zip (LGPL-2.1) as separate command-line helpers for winetricks.")
                                .font(.system(size: 12)).foregroundStyle(p.text)
                            HStack(spacing: 14) {
