...
/// library app through the managed Wine runtime — independent of MacWine.
enum AppExporter {
    @discardableResult
    static func export(app: WineApp, winePath: String, prefix: URL) throws -> URL {
        ...
        let script = """
        #!/bin/bash
        INFO="$HOME/Library/Application Support/MacWine/Wine/installed.json"
        ...
        <plist version="1.0"><dict>
          ...
          <key>CFBundleIdentifier</key><string>com.macwine.launcher.\(app.id)</string>
        ...
        // Icon: convert the app's custom/extracted icon to .icns, else reuse MacWine's.
